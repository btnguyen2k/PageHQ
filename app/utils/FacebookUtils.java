...
public class FacebookUtils {
	...
	public static Facebook getFacebook(String fbAccessToken) {
		return fbAccessToken != null ? new FacebookTemplate(fbAccessToken)
				: null;
	}

	private static boolean validateFbAccessToken(String fbAccessToken) {
		String cachedFbAccessToken = Controller
				.session(Constants.SESSION_FB_ACCESS_TOKEN);
	...
	public static List<FbPage> getFbPages(String fbAccessToken) {
		...
		if (validateFbAccessToken(fbAccessToken)) {
			result = (List<FbPage>) SessionUtils
					.getSession(Constants.SESSION_FB_PAGES);
		}
		if (result == null) {
			Facebook facebook = getFacebook(fbAccessToken);
			if (facebook == null) {
			...
			PagedList<Account> accounts = facebook.pageOperations()
					.getAccounts();
			...
			Collections.sort(result, new Comparator<FbPage>() {
				...
				public int compare(FbPage fbPage1, FbPage fbPage2) {
					...
					return result != 0 ? result : fbPage1.id
							.compareToIgnoreCase(fbPage2.id);
				}
			});
